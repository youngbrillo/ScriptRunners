...
void ECS::DrawWrappedText(
	Font font, std::string text, Rectangle rec, 
	float fontSize, float spacing, 
	Color fontColor, Color backgroundColor)
{

	//DrawTextEx( font, text.c_str(), Vector2{ rec.x, rec.y }, fontSize, spacing, fontColor 	);
	DrawRectangleRec(rec, backgroundColor);

	int length = TextLength(text.c_str());
	float textOffsetY = 0.0f, textOffsetX = 0.0f;
		...
		else
		{
			...
			else
			{
				...
				if ((codepoint != ' ') && (codepoint != '\t'))
				{
					...
					DrawTextCodepoint(font, codepoint, { rec.x + textOffsetX, rec.y + textOffsetY }, fontSize, fontColor);
				}
			}
